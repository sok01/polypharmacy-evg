...
Адаптировано под конкретную структуру графа и эффектов
"""
...
class ProbabilityOptimizer:
    ...
    def __init__(self, graph, target_effects):
        ...
        self.drug_map = self._find_drug_effects()
        self.initial_probs = load_combined_data()
        
        log("Инициализация оптимизатора:")
        log(f"- Узлов в графе: {len(graph['nodes'])}")
        log(f"- Связей в графе: {len(graph['links'])}")
        log(f"- Целевых эффектов для фозиноприла: {len(target_effects.get('фозиноприл', {}))}")
        
        self._prepare_optimization_data()
        self.bounds = [(0.0, 1.0) for _ in self.initial_params]
    
    def _find_drug_effects(self):
        """Находит соответствие между препаратом и его эффектами в графе"""
        drug_effects = defaultdict(list)
        
        # Находим узел препарата
        drug_node = next(
            (n for n in self.graph['nodes'] 
             if n.get('name', '').lower() == 'фозиноприл'), 
            None
        )
        
        if not drug_node:
            log("Препарат 'фозиноприл' не найден в графе!")
            return drug_effects
        
        # Находим все эффекты, связанные с препаратом
        effect_names = self.target_effects.get('фозиноприл', {}).keys()
        for effect_name in effect_names:
            ...
            if effect_node:
                drug_effects['фозиноприл'].append((effect_node['id'], effect_name))
        
        log(f"Найдено эффектов в графе: {len(drug_effects['фозиноприл'])}")
        return drug_effects
    ...
    def _prepare_optimization_data(self):
        ...
        # 1. Собираем параметры для всех узлов
        ...
        # 2. Создаем целевые пары (ID эффекта, вероятность)
        ...
        log(f"Подготовлено {len(self.target_pairs)} целевых пар для оптимизации")
