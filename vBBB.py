...
class BayesianNode:
    ...
    def _parse_probabilities(self, prob_data):
        # арсит данные вероятностей, преобразуя их в удобную для работы форму. добавил когда строил файл результатов. чтоб сеть считалась по структуре 
        parsed = {}
...
def generate_default_probabilities(graph_data):
    ...
    for node in graph_data['nodes']:
        ...
        if not parents:
            ...
            default_probabilities[node_name] = {"": 0}  # Пустая строка как ключ
        else:
            ...
            for comb in combs:
                ...
                state_probabilities[key] = 0.5  # Вероятность 1 для всех комбинаций
...
if __name__ == "__main__":
    graph_data = load_graph('drug_amlodipin_gr_oo.json')
